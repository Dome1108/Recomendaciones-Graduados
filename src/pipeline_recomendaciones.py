...
def cargar_graduados(ruta_excel):
    """
    Carga la base de graduados desde Excel y normaliza el identificador.
    """

    graduados = pd.read_excel(ruta_excel)
    graduados.columns = graduados.columns.str.strip()
...
def obtener_laboral_sql(conn, identificadores):
    ...
    ids = pd.Series(identificadores).dropna().astype(str).str.strip().unique().tolist()
    ...
    laboral["FECINGAFI"] = pd.to_datetime(laboral["FECINGAFI"], errors="coerce", dayfirst=True)
    ...
    laboral_unico = laboral.drop_duplicates(subset=["NUMAFI"], keep="last")
...
def detectar_columna_programa(catalogo):
    ...
    raise ValueError(
        "No se pudo detectar la columna del programa. Revisa catalogo.columns y define manualmente la columna."
    )
...
def generar_recomendaciones(base, catalogo_lineas):
    ...
    top3["Justificacion_Recomendacion_1"] = top3.apply(generar_justificacion, axis=1)
